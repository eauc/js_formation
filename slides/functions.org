...
** Closure
   ...
   function parentScope() {
     ...
     var anotherParentVar = "bye";

     function childScope(arg) {
       var childVar = " the ";
       return myParentVar + childVar + arg;
     }
   }
   ...
   |   var childVar = " the ";           +------+----------------+         +-------+
   |   return myGlobal + childVar + arg; |      |myParentVar     +---------+"hello"|
   | }                                   |      +----------------+         +-------+
   +-------------------------------------+      
   #+END_SRC
   ...
   |function myFunction(arg) {           |      |c5DF References |
   |   var childVar = " the ";           +------+----------------+         +--------+
   |   return myGlobal + childVar + arg; |      |myGlobal        +---------+"hello" |
   | }                                   |      +----------------+         +--------+
   +--------------+----------------------+      |arg             +---------+"world" |
                  |                             +----------------+         +--------+
                  |                          
                  v
   +-------------------------------------+      +----------------+
   |var childVar = " the ";              |      |c5DF References |
   |return myGlobal + childVar + arg;    +------+----------------+         +--------+
   +-------------------------------------+      |myGlobal        +---------+"hello" |
                                                +----------------+         +--------+
                                                |arg             +---------+"world" |
                                                +----------------+         +--------+
